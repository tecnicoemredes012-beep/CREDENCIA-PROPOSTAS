...
export async function generateProposalPDF(proposal: Proposal, settings?: SystemSettings | null) {
  const doc = new jsPDF({
  ...
  // Load logo
  const logoDataUrl = await getBase64Image(credenciaLogo);
  ...
  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, 'PNG', margin, currentY, 44, 15);
    } catch {
  ...
  // Create explicit Blob URL
  const blobUrl = URL.createObjectURL(pdfBlob);

  // In Chromium, anchor MUST be attached to DOM to respect download attribute,
  // otherwise Chrome treats it as a raw blob navigation and saves the UUID hash without .pdf extension!
...
export async function openProposalPDFInNewTab(proposal: Proposal, settings?: SystemSettings | null) {
  // Generate doc using existing function logic
  const fileName = await generateProposalPDF(proposal, settings);
  return fileName;
}
